...
<a href='inventory.php'><font color='white'>Back To Inventory</font></a>";
	...
	$r = sprintf("SELECT 
                    i.`itmid`,`itmimg`,`itmtype`,`itmimg`,`itmname`,`itmdesc`,`itmbuyprice`,`itmsellprice`,`weapon`,`armor`,
                    it.`itmtypeid`,`itmtypename` FROM `items` i 
                    LEFT JOIN `itemtypes` it 
                    ON i.itmtype = '%d' 
                    WHERE i.itmid = %d 
                    LIMIT 1", $itmtypeid, mysql_real_escape_string($itmid));
	$q = $db->query($r);
	if (!$db->num_rows($q)) {
		print "
