...
{
echo ' <h2> <font color=red>Assassination FAILED ! </font> </h2>  <br><br> <h4>  Your hired assassin is severely beaten by '.$assas['username'].'\'s bodyguard. </h4> ';
$h->endpage();
exit;
}
...
$db->query("UPDATE `users` SET `hospital`= {$hosp}, `hospreason`= 'Shot by an assassin.' WHERE `userid`={$assas['userid']}");
$db->query("UPDATE `users` SET `money`=`money`-{$cost2} WHERE `userid`={$ir['userid']}");
event_add($assas['userid'],"You were assassinated by someone.");
