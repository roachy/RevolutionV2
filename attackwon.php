...
{
//$stole=round($r['money']/(rand(200,5000)/5));
$all=$r['money'];
...
You have successfully mugged {$r['username']}!<br />
You knock {$r['username']} on the floor a few times to make sure he is unconscious, snatched his wallet and ran home.
{$r['username']} had $all in his wallet.";
$hosptime=rand(20,40)+floor($ir['level']/1);
$expgain=50;
$db->query("UPDATE users SET exp=exp+$expgain,money=money+$all WHERE userid=$userid");
$db->query("UPDATE users SET hp=1,money=0,hospital=$hosptime,hospreason='Mugged by <a href=\'viewuser.php?u={$userid}\'>{$ir['username']}</a>' WHERE userid={$r['userid']}");
event_add($r['userid'],"<a href='viewuser.php?u=$userid'>{$ir['username']}</a> mugged you and stole \$$all. Maybe you should put your money in the bank.",$c);
$atklog=mysql_escape_string($_SESSION['attacklog']);
$db->query("INSERT INTO attacklogs VALUES('',$userid,{$_GET['ID']},'won',unix_timestamp(),$all,'$atklog');");
