...
if ($_GET['ID'] == $userid)
	{
		print "<center>Don't attack yourself...
<br /><a href='index.php'>> Go Back <</a>";
		$h->endpage();
		exit;
	}
...
if ($ir['hp'] <= 1)
	{
		print "<center>You are unconscious, you can't move. 
<br /><a href='index.php'>> Go Back <</a>";
		$h->endpage();
		exit;
	}
else
if ($_SESSION['attacklost'] == 1)
	{
...
// Ripped from Revolution 2.0
// Makes it so users can't attack staff and vice versa.
...
if ($odata['bguard'] > 0)
	{
		print " <center> Upon trying to attack this man, you were brutally thrown to the ground by several gasmasked soldiers from the Blackwatch Corporation, they beat you with their weapons and left you in the hospital. You are now in the hospital for $hospit min<br/><br/><a href='index.php'>>Home</a></center>";
		$h->endpage();
		$db->query("UPDATE users SET hp=0, hospital=hospital+$hospit, hospreason='Dealt with by the Blackwatch Corporation' WHERE userid=$userid", $c);
		exit;
	}
...
<br /><a href='index.php'>> Go Back <</a>";
	...
	/*
	Want to pop people lol
	
	else if ($ir['user_level'] >= 2)
{
print "You are either trying to attack a staff member, or you are a staff member.<br>
To make the game fair, Revolution Development have stopped staff from attacking users.<br>
If you wish to resign as staff, please visit the staff menu and resign. <br>
 
<a href='index.php'>Back</a><br>
<a href='resign.php'>Resign from Staff</a>";
$h->endpage();
exit;
}*/
...
if ($youdata['hp'] <= 0 || $odata['hp'] <= 0)
...
else
	{
		...
		if($ir['fighttime'] == 0)
{
mysql_query("update users set fighttime=6 where userid=$userid");
}
 
if($ir['fighttime'] > 1)
{
print"<td colspan=2 align='center'><center>Your Attack Time Has ".$ir['fighttime']." Min(s) Remaining</center>";
}
if($ir['fighttime'] == 1)
{
mysql_query("update users set timeout=1 where userid=$userid");
}
if($ir['timeout'] == 1)
{
$time = rand(30,60);
print"<center>Your Slowness to move meant your opponent got bored and went home<a href='hospital.php'>Go Home</a></center>";
mysql_query("UPDATE users SET hospital=hospital+$time,hospreason='Failed to move in an attack!' WHERE userid=$userid",$c);
mysql_query("update users set fighttime=0 where userid=$userid AND userid=".$_GET['ID']."");
mysql_query("update users set timeout=0 where userid=$userid AND userid=".$_GET['ID']."");
$db->query("UPDATE users SET attacking=0 WHERE userid=$userid AND userid=".$_GET['ID']."");
$db->query("UPDATE users SET energy=energy-20 WHERE userid=$userid");
$db->query("UPDATE users SET attacking=0 WHERE userid=".$_GET['ID']."");
die();
}
