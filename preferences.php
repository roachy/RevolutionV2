...
// Fuck off, your coding style makes me want to kill myself.
...
<div class='pref_txt'><h1> Player Preferences </h1></div>
...
function do_sex_change()
{
	...
	$UserUpdate = sprintf("UPDATE users SET gender = '$g' WHERE (userid = %u)", $userid);
...
<a href='preferences.php?action=passchange'>> Back</a>";
		...
		$UserUpdate = sprintf("UPDATE users SET userpass=md5('{$_POST['newpw']}') WHERE (userid = %u)", $userid);
		mysql_query($UserUpdate);
...
<a href='preferences.php?action=namechange'> Back</a>";
		...
		$_POST['newname'] = str_replace(array(
			"<",
			">",
			"\\\'"
		) , array(
		...
		$UserUpdate = sprintf("UPDATE users SET username ='{$_POST['newname']}'  WHERE (userid = %u)", $userid);
		mysql_query($UserUpdate);
...
<a href='preferences.php?action=picchange'>> Back</a>";
		...
		if (!preg_match('~(.?).(jpg|jpeg|gif|png)~i', $_POST['newpic'])) {
			...
			$_POST['newpic'] = str_replace(array(
				"<",
				">",
				"'",
				";",
			...
			$userupdate = sprintf("UPDATE users SET display_pic = '{$_POST['newpic']}' WHERE userid = '%u' ", ($userid));
			mysql_query($userupdate);
			print "Pic changed!";
		}
	}
...
Any avatars that are not 140x140 will be automatically resized <form action='preferences.php?action=forumchange2' method='post'>
Avatar: <input type='text' class='form-control' name='forums_avatar' value='" . htmlspecialchars(stripslashes($ir['forums_avatar'])) . "' /><hr>
Signature (you may use BBcode): <textarea rows=10 cols=50 class='form-control' name='forums_signature'>" . $ir['forums_signature'] . "</textarea><br />
<input class='form-control' type='submit' value='Change Info' /></form>";
}
...
function do_forum_change()
{
	...
	$_POST['forums_avatar'] = str_replace(array(
		"<",
		">"
	) , array(
		"&lt;",
